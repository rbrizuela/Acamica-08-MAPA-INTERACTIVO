direccionesModulo = (function () {
  ...
  function calcularYMostrarRutas () {
    ...
    //obtener el cuadro de pantalla con sus valores
    var txtPuntos = document.getElementById('puntosIntermedios');

    //recorrer la lista y agregarlos al array de puntos Intermedios
    for ( var i = 0; i < txtPuntos.options.length; i++ ) {
      var latlngStr = txtPuntos.options[i].value.split(',', 2);
      var latlng = {lat: parseFloat(latlngStr[0]), lng: parseFloat(latlngStr[1])};
      puntosIntermedios.push({location: latlng,
                              stopover: false});
    }
     ...
     servicioDirecciones.route({
               ...
               waypoints: puntosIntermedios
                 ...
                 if (status === 'OK') {
                   ...
                   swal('Error de Localización','Imposible setear la ruta: ' + status,"error");
                 }
             });
